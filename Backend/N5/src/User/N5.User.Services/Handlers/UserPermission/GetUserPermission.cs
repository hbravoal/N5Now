...
[BrokerTopicHandler(EventUser.GetPermission)]
public class GetUserPermission : BrokerHandler, IBrokerEvent
{
    /// <summary>
    /// Mediator Instance
    /// </summary>
    ...
    public GetUserPermission(IBrokerConfiguration configuration, IMediator mediator)
        : base(configuration)
        => _mediator = mediator;
    ...
    public override async Task Handler(IBrokerPayload payload, CancellationToken token)
        => await _mediator.Send(
            new GetPermissionUseCase(payload.Value.ToDeserializeJSON<GetPermissionDto>())
        );
}